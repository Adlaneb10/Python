...
class Game(object):
    def __init__(self, high_num, player):
        ...
        # the highest possible number that can be chosen
        self.player = player
        # the user's name
        self.guessesTaken = 0
        # the initial number of guesses taken
        self.number = random.randint(1, self.high_num)
        # the number the computer chooses
    ...
    def play(self):
        print('Well, {},  I am thinking of a number between 1 and {}.'.format(str(self.player.name),self.high_num))
    ...
    # else: self.guess gets changed in get_guess function
        ...
        if self.guess ==self.number:
            ...
            print('Good job, {}! You guessed my number in {} guesses! Your highest score is {} guesses'.format(self.player.name, self.guessesTaken, self.player.get_highest_score()))
        else:
...
def main():
    ...
    print("right on, {} from {}! We've got 2 brands of game here.".format(player.name,player.country))

    while True:

        print("press 1 for easy 2 for difficult, or q to quit")
        user_choice = input()
